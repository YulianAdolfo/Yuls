...
func PrepareQueryForReport(containsOnlyErrors bool, typeDate int, dateStart, dateEnd string) string {
	...
	*/
	...
	if !containsOnlyErrors {
		query = fmt.Sprintf("SELECT * FROM %s LEFT JOIN TABLE_ERRORS ON %s = TABLE_ERRORS.ID_PATIENT WHERE %s BETWEEN '%s' AND '%s' ORDER BY %s ASC", DATABASE_IN_USE, dbId, tableAndField, dateStart, dateEnd, dbId)
	} else {
		query = fmt.Sprintf("SELECT * FROM %s INNER JOIN TABLE_ERRORS WHERE %s = TABLE_ERRORS.ID_PATIENT AND %s BETWEEN '%s' AND '%s' ORDER BY %s", DATABASE_IN_USE, dbId, tableAndField, dateStart, dateEnd, dbId)
	}
	return query
}
